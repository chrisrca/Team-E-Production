...
function getRandCol() {
    const colors = ["#EF4444", "#F6BD38", "#FFDB80", "#FFE9BF", "#A4D4FE", "#6D9FCD", "#6D9FCD", "#003365"];
            ...
            <ResponsiveContainer width="100%" height="100%">
                ...
                >
                    ...
                    {sources.map((source, index) => (
                        <Bar key={index} dataKey={dataKey} fill={getRandCol()} />
                    ))}
                </BarChart>
            </ResponsiveContainer>
        </div>
...
export function PiChart({ data, dataKey }: { data: ChartData[], dataKey: string }) {
    return (
        <div style={{ width: '100%', height: 400 }}>
            <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                    <Pie data={data} dataKey={dataKey} cx="50%" cy="50%" innerRadius={50} outerRadius={150} fill="#8884d8" label>
