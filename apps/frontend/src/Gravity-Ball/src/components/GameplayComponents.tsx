...
import { gameState, levelState, GameState, levelSeedState } from "./state";
import {
    cameraShiftX,
    cameraShiftY,
    cameraShiftZ,
...
const LevelObjects = () => {
    ...
    return (
        <>
            ...
            <Sector playerRef={playerRef} type="start" />
            {sectors.map((sector) => {
                const key = `${isPlaying}_${level}_${levelSeed}_${sector.x}_${sector.z}`;
                return <Sector key={key} playerRef={playerRef} {...sector} />;
            })}
            <Sector
...
export const GameplayComponents = () => {
    ...
    const cannonDebugProps = isPhysicsDebug
        ? { color: "green", scale: 1.01 }
        : {};
