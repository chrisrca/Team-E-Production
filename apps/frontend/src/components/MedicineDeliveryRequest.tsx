...
export default function DrugDelivery() {
  const drugOrderData = [];
  const [displayedDrugData, setDisplayedDrugData] = useState<DrugDeliveryData>({
    patientName: "",
    roomNumber: "",
    patientCondition: "",
    drugName: "",
  ...
  return (
    <>
        ...
        <div className="flex flex-wrap -mx-3 mb-6 items-end">
          ...
          <div className="w-full md:w-1/2 px-3">
            ...
            <FormInput
              ...
              onChange={(e: ChangeEvent<HTMLInputElement>) =>
                setDrugOrder({ ...drugOrder, drugQuantity: e.target.value })
              }
              placeholder="Enter Room Number..."
            />
          </div>
      ...
      <p>Patient Name: {displayedDrugData.patientName}</p>
    </>
  );
}
