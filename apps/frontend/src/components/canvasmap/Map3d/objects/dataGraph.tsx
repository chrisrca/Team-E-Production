...
const GraphMesh: React.FC<{ position: Vector3, pathNodes: DBNode[], level: number, nodes: DBNode[] }> = ({ position, pathNodes, level, nodes }) => {
    const [hoveredNode, setHoveredNode] = useState<string | null>(null);
    const floor = ["L2", "L1", "1", "2", "3"];
    if (!nodes) return;

    // Generating edges from the path
    const edges: Edge[] = pathNodes.map((node, index) => ({
        id: `edge-${index}`,
        start: node.nodeID,
        end: pathNodes[index + 1] ? pathNodes[index + 1].nodeID : node.nodeID
    })).slice(0, -1);
    ...
    return (
        <>
                ...
                if (node.floor == floor[level]) {
                    ...
                    return (
                        <mesh
                            key={index}
                            position={[
                                ((position.x - 50) + (node.xcoord / 50)), 0.1, ((position.z - 34) + (node.ycoord / 50))
                            ]}
                            scale={hoveredNode === node.nodeID ? [0.25, 0.25, 0.25] : [0.15, 0.15, 0.15]}
                            onPointerOver={() => setHoveredNode(node.nodeID)}
            ...
            {edges.map((edge, index) => {
                const startNode = nodes.find(node => node.nodeID === edge.start);
                const endNode = nodes.find(node => node.nodeID === edge.end);
                if ((startNode && endNode) && (startNode.floor == floor[level]) && (endNode.floor == floor[level])) {
                    return (
                        <Line
                            key={index}
                            points={[
                                [((position.x - 50) + (startNode.xcoord / 50)), 0, ((position.z - 34) + (startNode.ycoord / 50))],
                                [((position.x - 50) + (endNode.xcoord / 50)), 0, ((position.z - 34) + (endNode.ycoord / 50))]
                            ]}
                            color="orange"
                            lineWidth={10} 
                        />
